...
old_ppath     <- "M:\\Projects\\Northern Coin ELToDv2.2 2017-07"
new_ppath     <- "M:\\Projects\\Northern Coin ELToDv2.3 2017-0628"
output_dir    <- "M:\\Projects\\Northern Coin ELToDv2.3 2017-0628\\Analysis & Profiles"
...
# count scenarios (used when running scenario 2 alone)
for (p in 1:length(path)) {      # Both policies
  cnt <- 0
  
  for (s in 1:length(scenarios)){   # years (2020, 2040)
    cnt <- cnt + 1
    ...
    for (f in 1:length(filenames)){
      ...
      # read all files 
      df_vol <- read.csv(paste(path[p],"Base\\NorthernCoin",scenarios[s],filenames[f], sep ="\\"))
      ...
      ifelse(f == 1 & cnt == 1, df <- df_vol, df <- rbind(df,df_vol)) 
    }
  }
  ...
  for(s in 1:length(scenarios)){
    ...
    for(d in segments){
      # create excel output
      excel_out <- paste(output_dir, outfiles[[p]][s], sep ="\\") 
